...
Method: rolling 12-month median as expected value; Z-score = (actual - expected) / sigma.
...
def _build_surprise(months: int) -> dict:
    ...
    if current > 1.5:
        signal, color, stance = "数据持续超预期 UPSIDE SURPRISE", "#22c55e", "增长/盈利预期顺风"
    elif current > 0:
        signal, color, stance = "温和超预期 MILD UPSIDE", "#4ade80", "基本面边际改善"
    elif current > -1.5:
        signal, color, stance = "温和不及预期 MILD DOWNSIDE", "#fbbf24", "基本面动能偏弱"
    else:
        signal, color, stance = "数据持续不及预期 DOWNSIDE SURPRISE", "#ef4444", "增长下修压力升温"
    ...
    return {
        ...
        "stance": stance,
        "color": color,
        "insight": f"经济意外指数 {current:+.1f}，{stance}；需与流动性、利率和风险偏好交叉验证。",
