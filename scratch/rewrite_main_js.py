...
inject_replacement = '''async function injectTradeFromModal(action, qty) {
    ...
    try {
        ...
        closeActionModal();
        // Automatically switch to SIMU tab so user sees the result
...
# Replace injectTradeFromModal function entirely
text = re.sub(r'function injectTradeFromModal\(action, qty\)\s*\{.*?switchView\(\'simu\'\);\s*\}\s*\}', inject_replacement, text, flags=re.DOTALL)


# 2. Extract and Add renderExecutionHistory and startAuditTrailSync at the end of the file
polling_code = '''
...
# 3. We also need to strip out the inline rendering in initDecisionHub so they don't conflict, 
# or just let initDecisionHub call renderExecutionHistory(data.recent_executions)
